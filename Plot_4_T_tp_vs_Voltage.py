...
for i in range(1):
    plt.plot(Vol , T0p[i+2], color=colors[i], linewidth=3, ls=Ls[i])
    ax=plt.gca()
...
##plt.legend(title = "$E_{hote}=1.8eV$, $E_T=1eV$, $N=3x10^5cm^{-3}$, T=300K, $d_T=0.6nm$")


#plt.rcParams['title.labelweight'] = 'bold'
##plt.title('Trap time vs Voltage',fontweight="bold")
##ax.set_yscale('log')
